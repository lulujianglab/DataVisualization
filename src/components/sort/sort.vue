...
export default{
  ...
  watch: {
    ...
    diffNum(cur) {
      this.numArr = cur? cur : this.numArr
      this.getData()
    }
  },
  ...
  methods: {
    ...
    // 子组件传递值。
    showstartDate(data) {
      this.diffTime.startTime = data
    },

    showendDate(data) {
      this.diffTime.endTime = data
    },
    ...
    getData() { // 获取数据、处理数据
      axios.get('static/data/troubleData.json').then((res) => {
        ...
        if (this.diffData === 0) {
          ...
          if(this.numArr){
            let numlen = 500-100*(3-this.numArr)
            while(i++< numlen) {
          ...
          if(this.numArr){
            let numlen = 700-100*(3-this.numArr)
            while(i++< numlen) {
              newArr.push(geoData.random())
            }
          } else{
          ...
          if(this.numArr){
            let numlen = geoData.length-100*(3-this.numArr)
            while(i++< numlen) {
              newArr.push(geoData.random())
            }                      
          } else{
        ...
        this.rectangle_overlay? '' : this.dragSymbol()
